...
app.get('/api/bootstrap-static', async (req, res) => {
  try {
    ...
    // Process team fixtures and top performers
    for (const team of response.data.teams) {
      // Get next 5 fixtures for each team
      const teamFixturesResponse = await axios.get(`https://fantasy.premierleague.com/api/team/${team.id}/`);
      const nextFiveFixtures = teamFixturesResponse.data.fixtures.slice(0, 5).map(fixture => {
        const isHome = fixture.is_home;
        const opponent = response.data.teams.find(t => t.id === (isHome ? fixture.team_a : fixture.team_h)).short_name;
        return {
          opponent,
          isHome,
      ...
      // Get top 3 performers from this team in last 3 GWs
      const teamPlayers = response.data.elements.filter(p => p.team === team.id);
      const topPerformers = teamPlayers
        .map(player => ({
          name: player.web_name,
          points: player.points_per_game * 3, // Approximate last 3 GWs points
          photoId: player.code
        }))
